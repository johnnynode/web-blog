...
- 书籍的状态也可以用一个一维数组来描述：`int asigned[5];`
    ...
    * 若 assigned[book_id] == -1 则表示book_id这本书没有分配
    * 注意:数组下标是书的编号
    ...
    void Try(int reader) {
        ...
        for (int book = 0; book < 5; book++) {
            ...
            if ((like[reader][book] != 1) || assigned[book] != -1) {
                continue; // reader不喜欢book 或者 书籍已被分配，这时什么也不做, 去尝试下一本书
            }
            // 此时：reader 喜欢 book 并且 书籍未被分配
            // 记录当前这本书的分配情况
            assigned[book] = reader; // 标记当前书籍分配给了谁
            // 当前读者已分配书籍，为下一位读者分配合适书籍
            Try(reader + 1);
            // 注意，这里当一种方案已经敲定的时候才会进行状态的重置，也就是Try函数 return的时候(所有读者都有了阅读方案)，当所有读者都分配完成之后，将书退还(回溯)，尝试另一种方案
            assigned[book] = -1;
        }
    }
    ```
...
- 代码实现
    ...
    void Try(int reader, assign_state state) { 
        ...
        if (reader == 5) {
            num++;
            cout << "第" << num << "个方案(5本书的读者编号): ";
            for (int i = 0; i < 5; i++) cout << state.assigned[i] << ' '; cout << endl;
            return;
        }
...
<div align="center">
    <img width="400" src="./screenshot/16.jpg">
    <br />
    <div style="text-align:center">备注：图片托管于github，请确保网络的可访问性</div>
    <br />
</div>
...
<div align="center">
    <img width="400" src="./screenshot/17.jpg">
    <br />
    <div style="text-align:center">备注：图片托管于github，请确保网络的可访问性</div>
    <br />
</div>
