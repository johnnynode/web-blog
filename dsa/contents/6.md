...
总结：对于每一列, 找到左右两端的卡位hi和lo, 形成的矩形面积为：(hi - lo - 1) * hi, 可见这是一个线性的算法O(n)
...
- 分完之后, 还要考虑merge的方式，即左右和左右贯穿
    * (1) 计算 maxRect(h,lo,k) 局部矩形面积
    * (2) 计算 maxRect(h,k+1,hi) 局部矩形面积
    * (3) 计算 h[k] * (hi - lo - 1) 局部矩形面积
    * 找到上面三个的最大值作为局部最大值
- 此算法框架的复杂度取决于所使用的技术(还可以考虑使用线段树的相关知识减少复杂度，此处不做过多说明)
